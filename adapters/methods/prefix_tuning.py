...
from ..configuration import ModelAdaptersConfig, PrefixTuningConfig, BayesPrefixTuningConfig
...
class PrefixTuning(nn.Module, ModuleUtilsMixin):
    def __init__(
        ...
        self.control_trans = nn.Sequential(
            ...
            BayesLinear(prior_mu=config.prior_mu, prior_sigma=config.prior_sigma, in_features=self.input_size,
                        out_features=self.config.bottleneck_size, bias=False),
            ...
            # BayesLinear(prior_mu=0, prior_sigma=0.01, in_features=self.config.bottleneck_size,
            #             out_features=self.n_layers * 2 * self.n_heads * self.n_embd_per_head, bias=False),
        )
...
class PrefixTuningLayer(ComposableAdapterLayerBase, nn.Module):
    ...
    def compose_single(self, adapter_setup: str, state: PrefixTuningState, lvl: int = 0) -> PrefixTuningState:
        ...
        # prefix_keys=prefix_keys.to(state.key_states.device)
        # prefix_values=prefix_values.to(state.key_states.device)
