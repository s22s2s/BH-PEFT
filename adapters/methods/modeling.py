...
class Adapter(nn.Module):
    ...
    def __init__(
        ...
        if config["phm_layer"]:
            ...
            seq_list.append(BayesLinear(prior_mu=config.prior_mu, prior_sigma=config.prior_sigma, in_features=self.input_size,
                        out_features=self.down_sample, bias=False))
        ...
        if config["phm_layer"]:
            ...
            # self.adapter_up = BayesLinear(prior_mu=config.prior_mu, prior_sigma=config.prior_sigma, in_features=self.down_sample,
            #             out_features=self.input_size, bias=False)
            #只有down
        ...
        if config["init_weights"] == "bert":
            ...
            with torch.no_grad():
                ...
                if hasattr(self.adapter_down[0], 'weight'):
                    nn.init.kaiming_uniform_(self.adapter_down[0].weight, a=math.sqrt(5))
                    nn.init.zeros_(self.adapter_down[0].bias)
                # if hasattr(self.adapter_down[0], 'bias'):
                #     nn.init.zeros_(self.adapter_down[0].bias)

                if hasattr(self.adapter_up, 'weight'):
                    nn.init.zeros_(self.adapter_up.weight)
                    nn.init.zeros_(self.adapter_up.bias)
                else:
                    nn.init.zeros_(self.adapter_up.weight_mu)
                # if hasattr(self.adapter_up, 'bias'):
                #     nn.init.zeros_(self.adapter_up.bias)
                if self.use_gating:
                    self.gate.apply(self.init_bert_weights)
